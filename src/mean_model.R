mean_model = function(NSE,t_ahead)
{
  date = row.names(NSE)
  NSE =  NSE[,4]
  ...
  ######################################################################
  # FIT <- fitdist(retn[1:100], "norm")
  # 
  # png(file="distribution_modelling.png",width=1000,height=600)
  # plot(FIT)
  # dev.off()
  ...
  while(i<n1-t_ahead)
  {
    fit = fitdist(retn[(i-(days-1)):(i-1)],"norm")
    actual_volatality[i-(days-1)] = mean((retn[i:(i+t_ahead)] - fit$estimate[1])^2)
    predicted_volatality[i-(days-1)] = (fit$estimate[2]^2)*(1-lambda) + (retn[i-1]^2)*lambda
    predic_return[i-(days-1)] = fit$estimate[1]
  ...
  png(file="Distmodel_vol.png",width=1000,height=600)
  plot(out1[,2],
       type = "l",col = "blue",xlab = "time",ylab = "volatality",ylim = c(0,0.05),
       main = "Volatality vs Time",xaxt='n')
  ...
  # out1 = sqrt(out1)
  png(file="Distmodel_retn.png",width=1000,height=600)
  plot(out2[,2],
